...
class ControlUI(QWidget):
            ...
            if n == 4:
                ...
                if header == 0xA1:
                    ...
                    if state == 3:
                        alarm = b2
                        if alarm == 0:
                            alarm_txt = "HALL ALARM"
                        elif alarm == 1:
                            alarm_txt = "LOW VOLTAGE ALARM"
                        elif alarm == 2:
                            alarm_txt = "OVERLOAD ALARM"
                        elif alarm == 3:
                            alarm_txt = "PARAM ALARM"
                        elif alarm == 4:
                            alarm_txt = "OVERHEATING ALARM"
                        elif alarm == 5:
                            alarm_txt = "OVER VOLTAGE ALARM"
                        elif alarm == 6:
                            alarm_txt = "OVER SPEED ALARM"
                        elif alarm == 7:
                            alarm_txt = "OVERCURRENT ALARM"
                        else:
                            alarm_txt = "UNKOWN ALARM"
                    self._append_server_log(alarm_txt)
                else:
    ...
    def _send_dist(self):
        ...
        old_state = (self.mode, self.drive, self.speed)
        ...
        self.speed  = dist_lo
        self._send_frame()
        self._append_server_log(f"[DIST] {raw} m") # hi={dist_hi:02d}, mid={dist_mid:02d}, lo={dist_lo:02d}

        # Restore & resync buttons visibly
        self.mode, self.drive, self.speed = old_state
        self._sync_buttons()
        self._send_frame()
