...
class MotorController:
    ...
    def send_speeds(self, left_rpm: int, right_rpm: int):
        ...
        # t = time.monotonic()
        # with self.lock:
        #     self.left_speed = int(left_rpm)
        #     self.right_speed = int(right_rpm)

        #     # 최소 전송 간격
        #     if (t - self._last_tx_t) < self.MIN_TX_INTERVAL_S:
        #         return
        #     # 같은 값이면 무시
        #     if (self._last_tx_l, self._last_tx_r) == (self.left_speed, self.right_speed):
        #         return

        #     self._write_reg(self.SLAVE_LEFT,  self.REG_SPEED, self.left_speed)
        #     time.sleep(0.03)
        #     self._write_reg(self.SLAVE_RIGHT, self.REG_SPEED, self.right_speed)

        #     self._last_tx_l = self.left_speed
        #     self._last_tx_r = self.right_speed
        #     self._last_tx_t = t

        #     if self.debug:
        #         print(f"[RS485] send_speeds L={self.left_speed}, R={self.right_speed}")
        t = time.monotonic()
        # 1) 전송 여부/값 결정은 락 안에서
        with self.lock:
    ...
    def _tx_loop(self):
        ...
        while self._alive.is_set():
            ...
            with self.lock:
                ...
                #         self._last_tx_t = t
                #        # if self.debug:
                #        #     print(f"[RS485] TX(manual) L={self.left_speed} R={self.right_speed}")
                t = time.monotonic()
                need_send = False
