...
def motor_status_loop():
    ...
    while True:
        ...
        if abs(d_left) + abs(d_right) < THRESH_PULSES:
            ...
            # 평균 delta 부호로 전진/후진 판별
            avg = (d_left + d_right) / 2
            state = 1 if avg > 0 else 2  # 전진(1) / 후진(2)

        # 4) 상태 전송
        resp = bytes([0xA2, state, 0, 0])
        try:
            conn.sendall(resp)
        except:
            break
...
if __name__ == '__main__':
    ...
    threading.Thread(target=motor_status_loop, daemon=True).start()
    try:
        while True:
            time.sleep(1)
