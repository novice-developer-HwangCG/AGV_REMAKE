...
class LineTracer:
    ...
    '''def preprocess(self, roi):
        ...
        binary = cv2.bitwise_or(binary, flood_inv)
    ...
    def start_basic_tracing(self, speed_sel):
        ...
        # print(f"speed_sel : {self.auto_speed}")
        self.no_line = False
    ...
    def update_encoder(self, left_pulse, right_pulse):
        ...
        if self.target_distance_mm > 0:
            ...
            DECEL_ZONE = 300.0
            MIN_DECEL_SCALE = 0.3       # 감속 스케일 하한 (30%) = 감속 구간이 가까워 져도 기본 속도 이하로는 떨어지지 않도록
            FORCE_STOP_THRESH = 10.0    # 강제 정지 임계 (25mm)
            if 0 < remaining <= DECEL_ZONE:
                scale = remaining / DECEL_ZONE
                # 감속 스케일 하한 적용
                self.decel_scale = max(MIN_DECEL_SCALE, min(1.0, scale))
            else:
                self.decel_scale = 1.0
            # ----- 강제 정지 임계 -----
            if remaining < FORCE_STOP_THRESH:
