...
class LiDAR:
    ...
    def read_frame(self):
        ...
        if dist_raw < -32768:
            ...
            dist_signed = dist_raw

        hi = (dist_signed >> 8) & 0xFF
        lo = dist_signed & 0xFF

        # print(f"hi: {hi}")
        # print(f"lo: {lo}")
        # Protocol 반환: [0x5A, status, hi, lo] -> ui 쪽 복원 방법 signed 16-bit 정수로 복원 [예시 dist_signed = int.from_bytes(bytes([hi, lo]), byteorder='big', signed=True)]
        return bytes([0x5A, status, hi, lo])# hi, lo

    def reset_baseline(self):
        # 현재 프레임 읽어서 baseline 설정
        raw = self.read_frame()
        if raw and len(raw) == 4:
            status, hi, lo = raw[1], raw[2], raw[3]
            if status == 0:
                self.baseline = (hi << 8) | lo
