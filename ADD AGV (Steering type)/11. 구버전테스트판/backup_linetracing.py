        ...
        if self.motor.get_mode == 'auto':
            ...
            if self.auto_move == 0:
                ...
                if self.auto_move == 2:  # BWD
                    ...
                    err_steer = -error_n          # 후진은 부호 반전
                    vel_scale = max(0.4, 1.0 - 0.5 * min(abs(error_n), 1.0))
                    Kd *= 0.5
                    ...
                    step_limit = self.ACCEL_STEP
                # if self.auto_move == 2:
                #     # error = self.BWD_STEER_BIAS
                #     error = self.BWD_STEER_BIAS_LUT[self.auto_speed]
                #     Kd *= 0.5
                # else:
                #     # auto_move 값이 1일 때는 전진이니 error 값을 선에서 벗어나지 않도록 줄이기(?)
                ...
                if self.auto_move == 2:
                    #gL, gR = self.BWD_SIDE_GAIN_LUT.get(self.auto_speed, (1.03, 1.00))
