...
def pin_voltage_to_soc(v: float) -> int:
    """
...
def get_agv_status():
    global lidar, pico_st,motor_st, lidar_st
    try:
...
def send_state_on_command(conn, wait_ms: int = 100, force_state: int = None):
    global last_sent_state, last_adc, THRESH_PULSES, encoder
    ...
    if force_state in (0, 1, 2):
        ...
        if mag < max(THRESH_PULSES, 1):
            ...
            # 전진/후진 판정 보정: 오른(+), 왼(-)이면 전진 → dr - dl 기준
            s = dr - dl
