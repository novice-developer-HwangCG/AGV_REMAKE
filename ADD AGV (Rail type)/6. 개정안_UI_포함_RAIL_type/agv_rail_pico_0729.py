...
IP_ADDRESS = (192, 168, 100, 2)     # ip 수정 기존 10에서 2로 변경
...
MIN_DECEL_SCALE    = 0.3
ACCEL_STEP         = 10
FORCE_STOP_THRESH  = 0.5  # 기존 50 = 5cm, 변경 0.5 = 0.5mm 
...
class AGVServer:
    def __init__(self):
        ...
        self.power_led = digitalio.DigitalInOut(board.GP10)                 # 연결할 gp pin 확인 해서 수정
    ...
    def dispatcher(self):
        ...
        if mode == 2:
            hi, lo = cmd[1], cmd[2]
            raw_cm     = hi * 100 + lo        # e.g. 27, 44 → 2744cm
            self.target_dist = raw_cm * 10    # → 27440mm → 27.44m
            # 디버깅용으로 m 단위도 저장
            # self.ui_distance_m = meters
            self.encoder.reset()
            self.remaining = 0
            rame = bytes([FRAME_HEADER_DIST, hi, lo])
            sock.send(frame)
        ...
        if power_call =:
            ...
            resp = bytes([FRAME_HEADER_STATUS, 2, 0x00, bat])
            sock.send(resp)
            # if bat <= 2:
    ...
    # 모터가 어떤 상태에 있는지를 알려주기 위함 수정필요
    def motor_status_frame(self, conn):
        """
        drive 값에 따라 모터 제어 상태에 대한 프레임을 전송
        프레임 포맷: [0xA2, motor_status, 0x00, 0x00]
        motor_status : 0(정지), 1(전진), 2(후진)
        """
        """
        단순 모터가 움직이고 있는지 확인 하기 위한 코드
        현재 읽고 있는 엔코더 값이 이전 저장된 엔코더 값을 비교해서 엔코더 값이 이전과 다르다면 모터가 움직이고 있다는 뜻
        motor_state = self.encoder.distance_mm
        last_state = motor_state
        if motor_state != last_state:
            resp = bytes([0xA2, 1, 0x00, 0x00])
        else:
            resp = bytes([0xA2, 0, 0x00, 0x00])
        conn.send(resp)
        """
