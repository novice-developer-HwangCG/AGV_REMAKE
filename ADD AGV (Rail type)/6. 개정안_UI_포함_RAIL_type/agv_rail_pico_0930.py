...
ENCODER_MM_THRESHOLD = 0.5   # 움직임으로 인정할 최소 이동(mm) (튜닝)
...
class AGVServer:
    ...
    def dispatcher(self):
        ...
        if mode == FRAME_DISCONNECT:
            ...
            try:
                self.motor.stop()
                self.send_motor_state(sock, MOTOR_STOP)
            except:
                pass
            try:
                sock.close()        # 현재 연결만 종료
            except:
                pass
            return  # 내부 while 루프에서 recv 에러로 빠져나가고, 다시 accept() 대기
        ...
        if self.drive_mode == 3 and self.zsd_call == True:
            ...
            if abs(self.get_to_back_mm) < 25:    # 25mm 이내면 도착처리
                ...
                self.zsd_call = False
                self._prev_pos = self.encoder.enc.position
                # self.send_battery_bin(sock)         # [0xA1, 0x02, 0, bin]
                return
            else:
            ...
            try:
                while True:
                    ...
                    if self.target_dist > 0 and self.drive_mode in (1, 2):
                        ...
                        err = self.target_dist - traveled   # 목표 이동 거리 - 현재 이동 중인 거리 = 남은 거리 (예시. 1.00m - 0.50m = 0.50m 남은 거리 값이 임계 값 보다 작을 경우 도착 판정 정지) 
                        self.motor.set_ramp(
                            self.last_speed,
